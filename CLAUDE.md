...
**Current state:** scaffolded (declarative node, from the `declarative/github-issues` template, then rewritten for Precisely). The `Precisely` node has full read **and** write coverage across six resources — **Organization**, **Document**, **Project**, **Template**, **Reminder**, **Reviewer** — plus the `PreciselyApi` credential (X-API-KEY). `Roadmap.md` is the authoritative per-operation status list; keep it in sync as operations land. What remains: document/project **Signees**, document **Metadata Points** / **Links**, structured template `references` input, more top-level resources (Folders, Teams, Users), and the resthook **trigger**. `organizationId` is a dynamic dropdown backed by the `getOrganizations` loadOptions method.
