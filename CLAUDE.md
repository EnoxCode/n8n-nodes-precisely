...
**Current state:** scaffolded (declarative node, from the `declarative/github-issues` template, then rewritten for Precisely). The `Precisely` node covers **every endpoint (59/59)** under nine resources — **Organization**, **Document**, **Project**, **Template**, **Reminder**, **Reviewer**, **Signee**, **Metadata Point**, **Link** — plus the `PreciselyApi` credential (X-API-KEY). `Roadmap.md` is the authoritative per-operation status list; keep it in sync as operations land. What remains (backlog): more top-level resources (Folders, Teams, Users, Metadata Keys), structured UIs for the JSON body fields, and the resthook **trigger** node. `organizationId` is a dynamic dropdown backed by the `getOrganizations` loadOptions method.
